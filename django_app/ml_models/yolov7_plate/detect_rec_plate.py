...
def main():
    with open(os.path.join(settings.MEDIA_ROOT, 'config.json'), 'r') as config_file:
        config = json.load(config_file)

    # parser = argparse.ArgumentParser()
    # parser.add_argument('--detect_model', nargs='+', type=str, default=config['detect_model'], help='model.pt path(s)') #检测模型
    # parser.add_argument('--rec_model', type=str, default=config['rec_model'], help='model.pt path(s)')  #车牌识别 +颜色识别
    ...
    # parser.add_argument('--output', type=str, default=config['output'], help='source') 
    ...
    for pic_ in file_list:
        ...
        ori_img,str_=draw_result(img,dict_list)
        rand_path = "stdout" + str(np.random.rand()) + ".jpg"
        save_img_path = os.path.join(opt['output'],rand_path)# 只使用一张图片
        cv2.imwrite(save_img_path,ori_img)
        ans.append(str_)
        res.append(rand_path)
